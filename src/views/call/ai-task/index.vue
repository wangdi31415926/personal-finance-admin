...
const handleCreate = () => {
  router.push("/call/ai-task/create");
};

const handleEdit = (row: TaskRecord) => {
  router.push(`/call/ai-task/edit/${row.id}`);
};

const handleViewDetail = (row: TaskRecord) => {
  router.push(`/call/ai-task/detail/${row.id}`);
};
...
const handleConfirmExport = async () => {
  ...
  try {
    ...
    if (exportForm.format === "csv") {
      ...
      link.download = `任务数据_${dayjs().format("YYYY-MM-DD_HH-mm-ss")}.csv`;
      link.click();
    } else {
      // 生成Excel文件（这里简化处理，实际应该使用xlsx库）
      ElMessage.info("Excel导出功能需要安装xlsx库，当前使用CSV格式导出");
      const headers = exportForm.selectedFields.map(key => {
        const field = exportFields.find(f => f.key === key);
        return field?.name || key;
      });
      ...
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
      link.download = `任务数据_${dayjs().format("YYYY-MM-DD_HH-mm-ss")}.xlsx`;
      link.click();
    }
